...
impl Tetris {
    pub fn step(&mut self, input: Input) {
        ...
        if let Ok(output) = &result {
            ...
            {
                self.hard_drop_anim = Some(animation::HardDrop::new(locked_piece));
            }
        }
    }
    ...
    pub fn draw(&mut self, fb: &mut FrameBuffer) {
        ...
        for y in 0..height as i8 {
            for x in 0..width as i8 {
                let pos = Pos { x, y };
                if let Some(piece) = playfield.get(pos).flatten() {
                    let color = colors::piece(piece).darken(colors::DARKEN_STATIC_BLOCKS);
                    coordinates::PLAYFIELD.fill_block(fb, pos, color);
                }
            }
        }

        // Draw border
        coordinates::PLAYFIELD.fill_border(fb, colors::PLAYFIELD_BORDER);
        ...
        if let Some(ghost_pos) = self.game.ghost_piece_pos() {
            ...
            {
                coordinates::PLAYFIELD.fill_block(fb, pos, ghost_color);
            }
        }

        // Draw falling piece
        for pos in falling_piece.coordinates() {
            coordinates::PLAYFIELD.fill_block(fb, pos, falling_color);
        }
        ...
        animation::draw_opt(&self.lock_anim, fb, coordinates::PLAYFIELD);

        // Draw hard drop animation
        animation::draw_opt(&self.hard_drop_anim, fb, coordinates::PLAYFIELD);

        // Draw row clear animation
        animation::draw_opt(&self.clear_anim, fb, coordinates::PLAYFIELD);
    }
}
